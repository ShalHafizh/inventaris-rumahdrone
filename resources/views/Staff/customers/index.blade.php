...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function editForm(id) {
            ...
            $('#modal-form form')[0].reset();
            $.ajax({
                url: "{{ url('customers-staff') }}" + '/' + id + "/edit",
                type: "GET",
                dataType: "JSON",
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('customers-staff') }}";
                    else url = "{{ url('customers-staff') . '/' }}" + id;
